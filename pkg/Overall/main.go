...
func main() {
	...
	case 1:
		...
		switch {
		case writingType == "doom" || writingType == "doom.txt" || writingType == "Doom":
			typeOfWriting = "doom.txt"
		...
		fmt.Println(colorGreen + "\n-" + "Choisissez le positionnement de votre text ")
		fmt.Println(colorRed + "\nSi vous voulez plus d'information sur les alignements disponibles écrivez : " + colorWhite + "Aide" + colorGreen + normal)
		fmt.Print(colorYellowSombre+"\n"+"Quelle alignement choisissez-vous : ", normal)
		scanner = bufio.NewScanner(os.Stdin)
		scanner.Scan()
		alignement = scanner.Text()
		if alignement == "Aide" || alignement == "aide" {
			fmt.Println(colorCyan + "Il existe 4 types d'alignements: ")
			fmt.Println(colorYellowSombre + "Gauche\nMilieu\nDroite\nJustify ")
			fmt.Print("Quel type d'alignement choisissez-vous : " + normal)
			scanner = bufio.NewScanner(os.Stdin)
			scanner.Scan()
			alignement = scanner.Text()
		}
		switch {
		case alignement == "gauche" || alignement == "GAUCHE" || alignement == "Gauche":
			alignement = "left"
		case alignement == "milieu" || alignement == "MILIEU" || alignement == "Milieu":
			alignement = "center"
		case alignement == "droite" || alignement == "DROITE" || alignement == "Droite":
			alignement = "right"
		case alignement == "justify" || alignement == "JUSTIFY" || alignement == "Justify":
			alignement = "justify"
		default:
			fmt.Println(soulignerGrave + colorRedSombre + "Veuillez écrire correctement le style d'écriture" + normal)
			return
	...
	if alignement == "justify" {
		fmt.Print(couleur + fond)
		ascii_art.JustifyJustify(text, result, typeOfWriting, ascii_art.TerminalWidth())
		...
		if newLine {
			...
			for _, v := range splitText {
				...
				for i := 0; i < 8; i++ {
					...
					for _, v2 := range v {
						...
						result += ascii_art.Scanner(typeOfWriting, 1+int(v2-32)*9+i)
					...
					case "right":
						{
							fmt.Print(couleur + fond)
							fmt.Print(ascii_art.FillSpaces(ascii_art.TerminalWidth()-len(result)), result)
							result = ""
					...
					case "center":
						{
							fmt.Print(couleur + fond)
							fmt.Print(ascii_art.FillSpaces((ascii_art.TerminalWidth()-len(result))/2), result, ascii_art.FillSpaces((ascii_art.TerminalWidth()-len(result))/2), "\n")
							result = ""
			...
			for i := 0; i < 8; i++ {
				...
				for _, v := range text {
					...
					result += ascii_art.Scanner(typeOfWriting, 1+int(v-32)*9+i)
