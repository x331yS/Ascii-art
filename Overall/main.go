...
func Scanner(typeOfWriting string, num int) string {
	...
	var file, err = os.Open("./format/" + typeOfWriting)
...
func main() {
	...
	if alignement == "justify" {
		...
		justifyJustify(text, result, typeOfWriting, terminalWidth())
			...
			for _, v := range splitText {
				...
				for i := 0; i < 8; i++ {
					...
					for _, v2 := range v {
						...
						result += Scanner(typeOfWriting, 1+int(v2-32)*9+i)
...
func justifyLeft(text string, typeOfWriting string, result string)  {
...
func justifyJustify(text string, result string, typeOfWriting string, terminalWidth int)  {
	...
	if len(splitWSText) == 1 {
		...
		justifyLeft(text, typeOfWriting, result)
...
func terminalWidth() int {
	...
	if err1 != nil {
		switch language {
		case 1 :
			fmt.Println("Cela ne semble pas fonctionner....")
		case 2:
			fmt.Println("That doesn't work")
		case 3:
			fmt.Println("No puedo ....")
